...
class CurrentCycleView:
    ...
    def create_current_settings_section(self):
        ...
        self.cycles_entry = self._entry_with_label(row, 6, "Cycles:", self.cycles, self.save_entry_value,
                                                   Keys.CYCLES)
    ...
    def start_live_display(self):
        ...
        self.executor.submit(self.data_worker_loop)
    ...
    def update(self, running, timestamp, current, voltage, resistance):
        if not running:
        ...
        while self.running or not self.data_queue.empty():
            try:
                if self.data_queue.qsize() > Other.MAX_QUEUE_SIZE:
                    self.log(Logger.WARNING, "Queue backlog detected in live plotting!")
                timestamp, current, voltage = self.data_queue.get(timeout = 0.5)
                if self.running:
                    self.executor.submit(self.safe_plot, timestamp, current, voltage)
            except queue.Empty:
                continue
    ...
    def safe_plot(self, timestamp, y1, y2):
        try:
            ...
            self.log(Logger.ERROR, f"Plotting error: {e}\n {traceback.format_exc()}")
